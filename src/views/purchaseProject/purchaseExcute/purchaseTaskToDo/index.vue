<template>
  ...
  <div class="app-container">
    <task-select-filter @searchFun="emitHandleQuery"
                        searchPage="询价采购"></task-select-filter>
    ...
    <el-table v-loading="loading"
      ...
      <el-table-column label="部门联系人电话"
                       align="center"
                       prop="userPhone" />
      <el-table-column label="是否公开招标"
                       align="center"
                       prop="isOpen">
        <template slot-scope="scope">
          {{scope.row.endTime ? scope.row.isOpen === '1' ? '公开招标' : '非公开招标' : '未设置'}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button size="mini"
                     type="text"
                     icon="el-icon-edit"
                     @click="handleShow(scope.row)">询价</el-button>
          <el-button size="mini"
                     type="text"
                     icon="el-icon-edit"
...
export default {
  ...
  methods: {
    ...
    handleShow(row, updated) {
      console.log('row', row, `/purchaseExcute/purchaseTaskToDo/${row.id}/${updated}`)
      if (updated) {
        this.$router.push(`/purchaseProject/purchaseExcute/purchaseTaskToDo/detail/${row.id}/${updated}`)
        return;
      }
      this.$router.push(`/purchaseProject/purchaseExcute/purchaseTaskToDo/detail/${row.id}/空`)
    },
    /** 查询采购任务 列表 */
    getList() {
      this.loading = true;
