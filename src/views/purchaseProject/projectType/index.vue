  ...
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <el-form-item label="项目分类名称"
        ...
        <el-input v-model="queryParams.projectTypeName"
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="分类名称"
                          prop="projectTypeName">
              <el-input v-model="form.projectTypeName"
                        placeholder="请输入项目分类名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
import { listProjectType, getProjectType, delProjectType, addProjectType, updateProjectType, listProjectTypeExcludeChild } from "@/api/system/projectType";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      getProjectType(row.id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改项目分类";
      });
      listProjectTypeExcludeChild(row.id).then(response => {
        this.deptOptions = this.handleTree(response.data, "id");
      });
    },
    /** 提交按钮 */
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除名称为"' + row.projectTypeName + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delProjectType(row.id);
      }).then(() => {
        this.getList();
