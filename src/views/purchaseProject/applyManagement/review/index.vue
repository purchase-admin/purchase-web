  ...
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <el-form-item label="预算类型">
        <el-select v-model="queryParams.processDefinitionKey"
                   placeholder="请选择"
                   @change="handleQuery">
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        state: 'running',
        processInstanceName: '',
        processDefinitionKey: 'CGSQ'
      },
      keyTypes: [
        { label: '预算内', value: 'CGSQ' },
        { label: '预算外', value: 'CGSQYSW' },
      ],
      //数据字典-资金来源
      //级联配置
      cascaderOptions: [],//项目
  ...
  methods: {
    ...
    handleSee(row) {
      ...
      this.$router.push(`/purchaseProject/applyManagement/review/detail/` + id + '/' + row.businessKey + '/' + this.queryParams.state)
      //外部跳转-pc
      // this.$router.push(`/outer/applyManagement/review/detail/` + id + '/' + row.businessKey + '/' + this.queryParams.state)
      //外部跳转-mobile
      // this.$router.push(`/outer/mobile/applyManagement/review/detail/` + id + '/' + row.businessKey + '/' + this.queryParams.state)
    }
  },
  created() {
    this.getList()
